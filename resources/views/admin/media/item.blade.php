	...
	<div class="card" style="width: 18rem;">
		@if($file->media_type == 'Image')
			@if(auth()->user()->can($permissions['edit']))
	  			<a href="{{route($route.'.edit', [encrypt($file->id)])}}" class="webadmin-open-ajax-popup item-meida" title="Edit Image Details" data-popup-size="large"><img src="{{ asset($file->thumb_file_path) }}?ver={{time()}}"></a>
	  		@else
	  			<a href="{{route($route.'.show', [encrypt($file->id)])}}" class="webadmin-open-ajax-popup item-meida" title="View Image Details" data-popup-size="large"><img src="{{ asset($file->thumb_file_path) }}?ver={{time()}}"></a>
	  		@endif
	  	@else
