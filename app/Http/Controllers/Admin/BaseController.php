...
class BaseController extends Controller {
    ...
    protected function uploadFile($fileInput = 'image', $filePath = 'uploads/', $file_update_name=null, $varient=array()) {
        ...
        if ( (is_object($fileInput) || Input::hasFile($fileInput)) && $file->isValid()) {
            ...
            if(substr($file->getMimeType(), 0, 5) == 'image') {
                ...
                $width = (!empty($imagedetails[0]))?$imagedetails[0]:'NA';
                $height = (!empty($imagedetails[1]))?$imagedetails[1]:'NA';
            ...
            $success = false;
            $this->delete_if_exist($filePath, $fileName, $type);
            ...
            if($type == 'Image')
            {
                ...
                copy($destinationPath.$fileName, public_path('uploads/thumbnails/'.$fileName));
    ...
    public function delete_if_exist($destination, $file_name, $type)
    {
        ...
        if($type == 'Image'){
            $thumb_image = 'uploads/thumbnails/'.$file_name;
            if(file_exists(public_path($thumb_image))){
                @unlink(public_path($thumb_image));
            }
